...
/// This type will read data from a source, strip out whitespace and invalid UTF8
/// chars, and return what remains.
pub struct SpaceStripIter<T>
where
    T: Read,
{
    common: StreamIterCommon<T>,
...
impl<T> Iterator for SpaceStripIter<T>
where
    T: Read,
{
    type Item = io::Result<usize>;
    ...
    fn next(&mut self) -> Option<Self::Item> {
        ...
        loop {
            ...
            .into_owned();
            // remove any invalid utf8 characters
            buf = buf.replace(std::char::REPLACEMENT_CHARACTER, "");
            // remove whitespace by splitting into words and appending the words to each other
            for word in buf.split_whitespace() {
                let word_as_bytes = word.as_bytes();
                self.common.working_buf[read_previously..read_previously + word_as_bytes.len()]
                    .copy_from_slice(&word_as_bytes[..]);
                read_previously += word_as_bytes.len();
            }
            assert!(read_this_time <= self.common.working_buf.len());
        }
    }
}

impl_read_trait_for_stream_iter!(SpaceStripIter<T>);
...
#[cfg(test)]
mod test_space_strip_iter {
    use super::{SpaceStripIter, BUF_SIZE};

    #[test]
    fn empty_is_empty() {
        let output = SpaceStripIter::new(&vec![0u8; 0][..]).my_collect();
        assert_eq!(output.len(), 0);
    }

    #[test]
    fn just_invalid_is_empty() {
        let input = "\u{fffd}\u{fffd}\u{fffd}\u{fffd}\u{fffd}".as_bytes();
        let output = SpaceStripIter::new(&input[..]).my_collect();
        assert_eq!(output.len(), 0);
    }

    #[test]
    fn just_invalid_is_empty_long() {
        let mut input = vec![0xdd, 0xdd];
        while input.len() < BUF_SIZE * 2 + 1 {
            input.append(&mut vec![0xdd, 0xdd]);
        }
        let output = SpaceStripIter::new(&input[..]).my_collect();
        assert_eq!(output.len(), 0);
    }
    ...
    #[test]
    fn just_whitespace_is_empty_long() {
        let input = vec![' ' as u8; BUF_SIZE * 2 + 1];
        let output = SpaceStripIter::new(&input[..]).my_collect();
        assert_eq!(output.len(), 0);
    }

    #[test]
    fn invalid_and_whitespace_is_empty() {
        let input = " \u{fffd} \t\n\u{2009}\n\t".as_bytes();
        let output = SpaceStripIter::new(&input[..]).my_collect();
        assert_eq!(output.len(), 0);
    }
    ...
    #[test]
    fn invalid_and_whitespace_is_empty_long() {
        let mut input = vec![' ' as u8; BUF_SIZE];
        while input.len() < BUF_SIZE * 2 + 1 {
            input.append(&mut vec![0xdd, 0xdd]);
        }
        let output = SpaceStripIter::new(&input[..]).my_collect();
    ...
    #[test]
    fn leading_whitespace_long() {
        let content = "foo";
        // everything just less than buffer size
        let mut input = vec![' ' as u8; BUF_SIZE - content.len() - 1];
        input.append(&mut content.as_bytes().to_vec());
        assert_eq!(input.len(), BUF_SIZE - 1);
        let output = SpaceStripIter::new(&input[..]).my_collect();
        let output = String::from_utf8_lossy(&output);
        assert_eq!(output, content);
        // everything equal to buffer size
        let mut input = vec![' ' as u8; BUF_SIZE - content.len()];
        input.append(&mut content.as_bytes().to_vec());
        assert_eq!(input.len(), BUF_SIZE);
        let output = SpaceStripIter::new(&input[..]).my_collect();
        let output = String::from_utf8_lossy(&output);
        assert_eq!(output, content);
        // everything just greater than buffer size
        let mut input = vec![' ' as u8; BUF_SIZE - content.len() + 1];
        input.append(&mut content.as_bytes().to_vec());
        assert_eq!(input.len(), BUF_SIZE + 1);
        let output = SpaceStripIter::new(&input[..]).my_collect();
        let output = String::from_utf8_lossy(&output);
        assert_eq!(output, content);
        // content entirely after length of buffer size
        let mut input = vec![' ' as u8; BUF_SIZE];
        input.append(&mut content.as_bytes().to_vec());
        assert_eq!(input.len(), BUF_SIZE + content.len());
        let output = SpaceStripIter::new(&input[..]).my_collect();
        let output = String::from_utf8_lossy(&output);
        assert_eq!(output, content);
    }
    ...
    #[test]
    fn trailing_whitespace_long() {
        let content = "foo";
        // everything just less than buffer size
        let mut input = content.as_bytes().to_vec();
        input.append(&mut vec![' ' as u8; BUF_SIZE - content.len() - 1]);
        assert_eq!(input.len(), BUF_SIZE - 1);
        let output = SpaceStripIter::new(&input[..]).my_collect();
        let output = String::from_utf8_lossy(&output);
        assert_eq!(output, content);
        // everything equal to buffer size
        let mut input = content.as_bytes().to_vec();
        input.append(&mut vec![' ' as u8; BUF_SIZE - content.len()]);
        assert_eq!(input.len(), BUF_SIZE);
        let output = SpaceStripIter::new(&input[..]).my_collect();
        let output = String::from_utf8_lossy(&output);
        assert_eq!(output, content);
        ...
        // content entirely after length of buffer size
        let mut input = content.as_bytes().to_vec();
        input.append(&mut vec![' ' as u8; BUF_SIZE]);
        assert_eq!(input.len(), BUF_SIZE + content.len());
        let output = SpaceStripIter::new(&input[..]).my_collect();
        let output = String::from_utf8_lossy(&output);
        assert_eq!(output, content);
...
impl_tests_for_common_read!(test_read_common_with_space_strip_iter, SpaceStripIter);
